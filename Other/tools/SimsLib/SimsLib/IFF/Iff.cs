...
namespace SimsLib.IFF
{
    ...
    public class Iff
    {
        private string m_name, m_Path, versions;
        private BinaryReader m_Reader;
        private int pmsr, size;
        private uint rsmpOffset, version, types;
        ...
        private List<FAMI> m_FAMIs = new List<FAMI>();
        private List<FAMh> m_FAMhs = new List<FAMh>();
        private List<NBRS> m_NBRs = new List<NBRS>();
        private List<SIMI> m_SIMIs = new List<SIMI>();
        private List<HOUS> m_Hous = new List<HOUS>();
        private List<ARRY> m_Arrys = new List<ARRY>();
        ...
        public List<BMP_> BMps
        {
            get { return m_BMP_s; }
        }

        public List<FAMI> FAMIs
        {
            get { return m_FAMIs; }
        }

        public List<FAMh> FAMhs
        {
            get { return m_FAMhs; }
        }

        public List<NBRS> NBrs
        {
            get { return m_NBRs; }
        }

        public List<SIMI> SIMIs
        {
            get { return m_SIMIs; }
        }

        public List<HOUS> HOUSs
        {
            get { return m_Hous; }
        }

        public List<ARRY> Arrys
        {
            get { return m_Arrys; }
        }
        ...
        public Iff(string Dir)
        {
            m_Path = Dir;
            m_name = Path.GetFileNameWithoutExtension(Dir);
        ...
        {
            m_Reader = new BinaryReader(Data, Encoding.ASCII);
        ...
        private void ReadChunks()
        {
            ...
            rsmpOffset = Endian.SwapUInt32(m_Reader.ReadUInt32());
                ...
                foreach (int offset in file.Value)
                {
                    if (offset > 0)
                    {
                        ...
                        if (StrResource == "SPR#" || StrResource == "SPR2" || StrResource == "rsmp" || StrResource == "PALT" ||
                            ...
                            StrResource == "BMP_" || StrResource == "FCNS" || StrResource == "FAMI" || StrResource == "FAMs" ||
                            StrResource == "SIMI" || StrResource == "FAMh" || StrResource == "NBRS" || StrResource == "HOUS" ||
                            StrResource == "Arry")
                        {
        ...
        private void ParseStrings()
        {
            foreach (IffChunk Chunk in m_Chunks)
            {
                if (Chunk.Resource == "STR#" || Chunk.Resource ==  "TTAs" ||
                    Chunk.Resource == "CTSS" || Chunk.Resource == "CST" || Chunk.Resource == "FAMs")
        ...
        private void ParseFAMI()
        {
            foreach (IffChunk Chunk in m_Chunks)
            {
                if (Chunk.Resource == "FAMI")
                    m_FAMIs.Add(new FAMI(Chunk));
            }
        }

        private void ParseFAMh()
        {
            foreach (IffChunk Chunk in m_Chunks)
            {
                if (Chunk.Resource == "FAMh")
                    m_FAMhs.Add(new FAMh(Chunk));
            }
        }

        private void ParseNBRS()
        {
            foreach (IffChunk Chunk in m_Chunks)
            {
                if (Chunk.Resource == "NBRS")
                    m_NBRs.Add(new NBRS(Chunk));
            }
        }

        private void ParseSIMI()
        {
            foreach (IffChunk Chunk in m_Chunks)
            {
                if (Chunk.Resource == "SIMI")
                    m_SIMIs.Add(new SIMI(Chunk));
            }
        }

        private void ParseHOUS()
        {
            foreach (IffChunk Chunk in m_Chunks)
            {
                if (Chunk.Resource == "HOUS")
                    m_Hous.Add(new HOUS(Chunk));
            }
        }

        private void ParseARRY()
        {
            foreach (IffChunk Chunk in m_Chunks)
            {
                if (Chunk.Resource == "Arry")
                    m_Arrys.Add(new ARRY(Chunk));
            }
        }
