...
#define MAXSIZE_FAR			MINSIZE_DBPFFILE
#define MINSIZE_FARENTRY	MINSIZE_FAR1ENTRY
#define MAXSIZE_FARENTRY	MINSIZE_FAR3ENTRY
...
#define DECLSPEC   __declspec( dllexport ) 
	...
	DECLSPEC enum FARTYPE libfarIdentify(const unsigned char * buffer, unsigned int filesize);
	DECLSPEC int CreateFAR1File(struct FAR1File * File, const unsigned char * buffer, unsigned int filesize);
	DECLSPEC int CreateFAR3File(struct FAR3File * File, const unsigned char * buffer, unsigned int filesize);
	DECLSPEC int CreateDBPFFile(struct DBPFFile * File, const unsigned char * buffer, unsigned int filesize);
	DECLSPEC int FARInitialize(void * File, enum FARTYPE ArchiveType, unsigned int FilesCount, const unsigned char * Index, unsigned int IndexSize);
	DECLSPEC int CreateFAR1Entry(struct FAR1Entry * Entry, const unsigned char * buffer);
	DECLSPEC int CreateFAR3Entry(struct FAR3Entry * Entry, const unsigned char * buffer);
	DECLSPEC int CreateDBPFEntry(struct DBPFEntry * Entry, const unsigned char * buffer);
	DECLSPEC int CreateRefPackStream(struct RefPackStream * Stream, const unsigned char * buffer);
	DECLSPEC int RefPackDecompress(const struct RefPackStream * Stream, const unsigned char * inbuffer, unsigned char * outbuffer);
	#if (defined FAR_WIN32 || defined FAR_POSIX)
	DECLSPEC unsigned int FARExtractItemFromFileByID(const char * archivename, unsigned int FileID, unsigned char ** Destination, int Export);
	DECLSPEC unsigned int FARExtractItemFromFileByName(const char * archivename, const char * EntryFilename, unsigned char ** Destination, int Export);
	#endif
