...
| S3 | Fetch `Content/OGL` XNBs; load effects (start **iOS/GLVer=2**) | **Partial DONE:** CI rebuilt `colorpoly2D.fx` → **KNIF** XNB (`XNBb` / `KNIF` v11) at `BrowserClient/wwwroot/Content/Effects/colorpoly2D.xnb` via `PackTools/FSO.BrowserEffects` + `kni-effects-blazor.yml`. BrowserClient `Content.Load` prefers it (teal status); BasicEffect fallback if missing. Stock FreeSO MGFX 11 still fails (`?lot=1&effect=1`). **Mac cannot build** (needs `d3dcompiler_47.dll`). Remaining for LotView: rebuild `GrassShaderiOS`, `2DWorldBatchiOS`, etc. |
| S4 | Retarget HIT; one SoundEffect under autoplay rules | Audio beep |
| S5 | LotView + Vitaboy; empty lot | Lot camera + floor — **placeholder DONE** (`?lot=1`) + Mario optional (`FSO_NO_SM64`/`BLAZORGL`) + MapGenerationiOS fallback + LotView closure `net8;net9` + **BrowserClient ProjectReference** (`FSO_GRAPHICS=Kni`, `FSO_NO_SM64`) + **`?lot=real` → ExternalWorld + flat grass** (diamond fallback on fail). KNIF lot FX under `wwwroot/Content/Effects/`. Runtime pixels = best-effort; prove in browser. |
