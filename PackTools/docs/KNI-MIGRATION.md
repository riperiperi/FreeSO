...
| S5 | LotView + Vitaboy; empty lot | Lot camera + floor — **placeholder DONE** + Mario optional (`FSO_NO_SM64`/`BLAZORGL`) + MapGenerationiOS fallback + **LotView closure dual-targeted `net8.0;net9.0`** (Common/Files/Content/HIT/Vitaboy*/LotView/TargaImagePCL). Real `ExternalWorld` still needs full KNIF lot FX set + BrowserClient ProjectReference (not wired yet). |
