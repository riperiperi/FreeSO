...
- **Still open, in order:** (1) VM tick streaming so joined clients share state/avatars (real Phase B), (2) swap fake servers for the real archive server, (3) ~~XNB byte-patch~~ **RETIRED 2026-08-13 (remote session): patched bytes committed** (`patch_glsl_es.py`, now also kills the `#elif GL_EXT_gpu_shader4` SwiftShader rejects), (4) real object pipeline in browser — **blocked on a KNIF effect rebuild, fx source fixed, CI round pending**, see ledger below, (5) Vitaboy avatars in browser, (6) MTL-color import bug (green toilet), (7) desktop grove screenshot, (8) SLOT/sit.
...
**Fix in flight:** `2DWorldBatchiOS.fx` source patched (distinct `WorldViewProjection`/`RotProjection` semantics; `vsSimple` consumes the full 5-attr layout) in both copies; push triggers `kni-effects-blazor.yml` → rebuilt XNBs → `patch_glsl_es.py` → commit → flip `?furnish=` default back to `real` and drop the two gated workarounds (`_2DWorldBatch.AliasedMatrixWorkaround`, drawZSprite+depthOut default). Until then billboards stay the visible default; `RealFurnitureLayer` (engine DGRP data, direct batch draw) is ready behind `?furnish=real`.
