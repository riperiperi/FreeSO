...
`Properties/launchSettings.json`). Dark-blue canvas + centered FreeSO-branded panel.
...
1. Keep this BlazorGL host; grow game code inside `FSO.BrowserClientGame`.
2. Retarget FreeSO client packages from `MonoGame.Framework.DesktopGL` → matching
   `nkast.Xna.Framework.*` (same APIs KNI already exposes here).
3. Content over HTTP (FAR / assets), not local disk.
4. Speak Aries over the gateway (reuse `FSO.Server.Protocol` in WASM, or grow from the
   JS seed in `FSO.WsGateway/wwwroot`).

See `../docs/BROWSER-VIABILITY.md` and root `task_plan.md` Phase F.
