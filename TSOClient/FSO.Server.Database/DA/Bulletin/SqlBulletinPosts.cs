...
namespace FSO.Server.Database.DA.Bulletin
{
    public class SqlBulletinPosts : AbstractSqlDA, IBulletinPosts
    {
        ...
        {
            return Context.Connection.Query<DbBulletinPost>("SELECT * FROM fso_bulletin_posts WHERE neighborhood_id = @neighborhood_id AND date > @timeAfter", 
                new { neighborhood_id, timeAfter }).ToList();
        }

        public int CountPosts(uint neighborhood_id, uint timeAfter)
        {
            return Context.Connection.Query<int>("SELECT count(*) FROM fso_bulletin_posts WHERE neighborhood_id = @neighborhood_id AND date > @timeAfter",
                new { neighborhood_id, timeAfter }).FirstOrDefault();
        }

        public uint LastPostID(uint neighborhood_id)
        {
            return Context.Connection.Query<uint>("SELECT bulletin_id FROM fso_bulletin_posts WHERE neighborhood_id = @neighborhood_id ORDER BY bulletin_id DESC LIMIT 1",
        ...
        public DbBulletinPost LastUserPost(uint user_id)
        {
            return Context.Connection.Query<DbBulletinPost>("SELECT b.* FROM fso_bulletin_posts b " +
                "JOIN fso_avatars a ON b.avatar_id = a.avatar_id WHERE a.user_id = @user_id " +
                "ORDER BY b.date DESC " +
                "LIMIT 1",
                new { user_id }).FirstOrDefault();
        }
    }
}
