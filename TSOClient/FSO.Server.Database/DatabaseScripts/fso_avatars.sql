CREATE TABLE `fso_avatars` (
  ...
  `lot_permissions` smallint(5) unsigned NOT NULL DEFAULT '0',
  ...
  `lockpoints` smallint(5) unsigned NOT NULL DEFAULT '0',
  ...
  CONSTRAINT `FK_lot` FOREIGN KEY (`lot_id`) REFERENCES `fso_lots` (`lot_id`) ON DELETE SET NULL ON UPDATE CASCADE,
...
CREATE TABLE `fso_relationships` (
