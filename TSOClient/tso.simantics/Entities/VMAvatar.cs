        ...
        */
        ...
        public override void Tick()
        {
            ...
            if (Thread != null)
            {
                ...
                if (!Content.Content.Get().TS1 && Position == LotTilePos.OUT_OF_WORLD && (PersistID > 0 || IsPet) && 
                    (Container == null || Container.Position == LotTilePos.OUT_OF_WORLD) && Thread.IsQueueIdle())
                {
                    // FreeSO failsafe: Sims that are out of bounds + idle get snapped back to the spawn location
        ...
        public SurroundPuppet GetSurroundPuppet(bool withMessage)
        {
            ...
            return new SurroundPuppet()
            {
                ...
                Message = withMessage ? new SurroundPuppetMessage(Message, Name, MessageTimeout, tsoState?.ChatColor.PackedValue ?? 0u, tsoState?.ChatTTSPitch ?? 0, PersonData[(int)VMPersonDataVariable.Gender] > 0) : default
            };
        }
