...
namespace TSOClient.Code.Rendering.Lot.Components
{
    public class TerrainComponent : House3DComponent
    {
        ...
        public TerrainComponent(HouseRenderState state)
        {
            ...
            var tl = state.GetWorldFromTile(new Vector2(0, 0));
            var tr = state.GetWorldFromTile(new Vector2(state.Size, 0));
            var bl = state.GetWorldFromTile(new Vector2(0, state.Size));
            var br = state.GetWorldFromTile(new Vector2(state.Size, state.Size));
