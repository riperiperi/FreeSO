...
namespace FSO.Windows.Platform
{
    ...
    public partial class WindowsMultimediaTimerResolution : IDisposable
    {
        ...
        private void EnsureResolutionSupport()
        {
            ...
            if (result != 0)
            {
                Console.WriteLine($"timeGetDevCaps failed with result: {result}");
            }
            else
            {
                ...
                {
                    Console.WriteLine($"Target resolution isn't supported by OS, using closest resolution: {supportedTargetResolutionInMilliseconds}ms");
        ...
        private void Activate()
        {
            ...
            if (result != 0)
            {
                Console.WriteLine($"timeBeginPeriod failed with result: {result}");
            }
            else
            {
                _isActive = true;
        ...
        private void Disable()
        {
            if (_isActive)
            {
                ...
                if (result != 0)
                {
                    Console.WriteLine($"timeEndPeriod failed with result: {result}");
                }
                else
                {
                    _isActive = false;
