...
ZVertexOut vsZSprite(ZVertexIn v){
    ZVertexOut result;
    float4 position = mul(v.position, viewProjection);
    result.position = position;
    result.screenPos = position.xy * float2(0.5, -0.5) + float2(0.5,0.5);
    result.texCoords = v.texCoords;
	result.objectID = v.objectID;
...
void psZDepthSprite(ZVertexOut v, out float4 color:COLOR0) {
    ...
    //SOFTWARE DEPTH TEST
    if (depthOutMode == false && unpackDepth(tex2D(depthMapSampler, v.screenPos)) < depthT) discard;

    float4 depthB = packDepth(depthT);
...
void psSimpleRestoreDepth(ZVertexOut v, out float4 color: COLOR0){
	...
	if (color.a < 0.01) {
		...
		float depthT = v.backDepth + unpackDepth(dS);
        if (depthOutMode == false && unpackDepth(tex2D(depthMapSampler, v.screenPos)) < depthT) discard;
        if (depthOutMode == true) {
            color = packDepth(depthT);
        }
